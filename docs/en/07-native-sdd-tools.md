...
Running Spec-kit end-to-end on a real feature reveals a five-stage pipeline — `constitution → specify → clarify → plan → tasks` — each producing persistent artifacts. For a moderately scoped feature (a photo album organizer with five user stories), the full run generates eight interconnected files totaling over a thousand lines of markdown. That's significant, but context matters: in projects where multiple contributors review specifications before implementation begins, this volume serves as a shared decision record rather than overhead. The generated API contracts, for instance, function as an explicit interface agreement between frontend and backend — useful when both layers are implemented in the same codebase by the same agent, because they prevent drift between what the API serves and what the UI consumes.

The strongest step in the pipeline is **clarification**: a structured round of at most five multiple-choice questions with reasoned recommendations, designed to surface data-model ambiguities, UX gaps, and missing functional requirements before planning starts. It's chapter 5's iterative clarification turned into an effective, bounded mechanism — and it's the step that most directly reduces downstream rework.

The most notable gap is what happens *after* implementation. Spec-kit has no built-in mechanism to verify that generated code actually satisfies the specification. The pipeline ends at task generation; from there, the spec and the code evolve independently with no automatic drift detection. This is the core reason it remains spec-first rather than spec-anchored: the feedback loop from code back to spec doesn't exist. A checklist or recurring validation agent that compares acceptance criteria against test results would close the gap — and it's the kind of infrastructure that chapter 12's harness covers.

A second gap surfaces when you add a second feature. Each spec directory is fully self-contained — its own data model, its own API contracts, its own research decisions, its own quickstart. Neither the `specify` nor the `plan` commands instruct the agent to read existing specs from prior features. The constitution is the only artifact that lives across features, but it captures abstract principles, not concrete implementation decisions (which database, which entities already exist, which endpoints are already defined). In practice, capable agents compensate: when running the pipeline for a second feature on the same codebase, Claude Code explored the existing spec directory on its own initiative — using its tools to read prior artifacts — correctly extended the data model instead of redefining it, and produced a quickstart that said "no additional setup beyond Feature 001." But this happened because the agent was capable enough to decide to look — the pipeline didn't tell it to. With a less capable agent, a larger project with dozens of prior specs, or a session under context pressure, the same pipeline could produce a second feature that redefines the tech stack from scratch. A project-level registry of accumulated decisions (entities, tech stack, contract catalog) would make this behavior deterministic rather than probabilistic.
