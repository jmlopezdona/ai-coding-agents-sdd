...
If you've made it this far, you have SDD as a discipline: you know how to specify intent, maintain the bidirectional loop, modulate the process by task type, and you have judgment to not confuse SDD with bureaucracy. What's missing is understanding **where this connects** with the trilogy's next layer: the **harness**, the internal infrastructure system that surrounds the agent so SDD scales to a whole team and to months at a time.
...
The harness's second pillar is the duality **guides and sensors**: artifacts that orient the agent *before* acting (guides) and verifiers that detect when the agent has drifted *after* (sensors). The chapter 4 SDD cycle's phase 5 — verification — is exactly a kind of sensor.
...
This is the technical way to close the chapter 5 bidirectional loop. Without harness, the loop depends on human discipline ("remember to update the spec"). With harness, the loop is automatic ("the sensor detected code and spec diverged and opened an issue to reconcile them").

## Coupling point 3 — Hooks that keep specs alive

The chapter 6 Kiro detail — the **hooks that fire on file save** — isn't a detail. It's the prefiguration of one of the central harness mechanisms: **repo events that trigger agent actions** (regenerate docs, update indexes, validate invariants, refresh the living-specs loop).
...
The chapter 7 tools — Traycer and similar — are technically **harness applied to SDD**. What they do — intercepting inputs, planning, verifying outputs — is exactly the harness guides-and-sensors dynamic, applied to the concrete cycle of a session with an agent.
...
Several chapter 11 anti-patterns are specifically hard to avoid without harness:
...
In other words: **a significant part of what kills bad SDD is exactly what the harness is designed to solve**. The two courses aren't sequential by whim — they're sequential because the problems you discover in well-done SDD are the problems harness addresses.
...
- **The chapter 5 bidirectional loop works when someone takes care of it and breaks when nobody looks.** That's a sign it needs to go from human process to infrastructure.
- **Chapter 11 anti-patterns reappear periodically** even though the team knows them. Individual awareness doesn't scale; infrastructure does.
- **Chapter 6 tools fall short because they're single-dimension** and your system needs multiple layers (sensors + hooks + sandboxes + observability). That's a full harness, not one more tool.
...
- **The loop as primitive.** Harness ch. 4 presents "iteration as primitive". This course's ch. 4 cycle is a particular case.
...
Second, **bad SDD is indistinguishable from bureaucracy and sometimes worse**. This is also documented and is why chapter 9 exists. Applying SDD without judgment turns the discipline into its own bottleneck, and within weeks the team goes back to vibe coding with an additional layer of cynicism.
